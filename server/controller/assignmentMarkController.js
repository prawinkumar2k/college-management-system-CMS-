...
// Get distinct assignment numbers (Test_No) with dates and max marks
...
export const checkMarksEntered = async (req, res) => {
  try {
    ...
    if (!courseName || !deptName || !deptCode || !semester || !regulation || !section || !subjectCode || !assignmentNo || !assignmentDate) {
      return res.status(400).json({ error: 'All filter parameters are required' });
    }
    ...
    const [result] = await pool.query(`
      ...
      WHERE Course_Name = ?
        ...
        AND Assignment_No = ?
        AND Assessment_Date = ?
    `, [courseName, deptCode, deptName, semester, regulation, section, subjectCode, assignmentNo, assignmentDate]);
...
export const getStudents = async (req, res) => {
  try {
    ...
    if (!courseName || !deptName || !semester || !regulation || !section || !assignmentNo || !assignmentDate || !subjectCode) {
      return res.status(400).json({ error: 'All filter parameters including subject code, assignment number and date are required' });
    }

    // Query assessment_students table joined with assignment_mark_entered to get existing marks if any
    const [students] = await pool.query(`
      ...
      LEFT JOIN assignment_mark_entered m ON 
...
export const saveAssignmentMarks = async (req, res) => {
  ...
  try {
    ...
    if (!courseName || !deptCode || !semester || !regulation || !section || !subjectCode || !assignmentNo || !enteredBy) {
    ...
    for (const mark of processedMarks) {
      ...
      // Validate marks: 'A' or numeric within range
      ...
      // Check if record exists for this student and assignment
      const [existing] = await connection.query(`
        SELECT 1 FROM assignment_mark_entered
      ...
      if (existing.length > 0) {
        ...
        await connection.query(`
          UPDATE assignment_mark_entered SET
            Obtained_Mark = ?,
            Max_Marks = ?,
            Entered_By = ?,
            UpdatedAt = NOW()
        ...
        // Insert new record
        await connection.query(`
          INSERT INTO assignment_mark_entered(
            Register_Number,
            Name,
            Course_Name,
            Dept_Code,
    ...
    res.json({
      message: `Assignment marks saved successfully for ${marks.length} students`,
      count: marks.length
    });
  } catch (error) {
    await connection.rollback();
    console.error('Error saving assignment marks:', error);
    res.status(500).json({ error: 'Failed to save assignment marks' });
  } finally {
    connection.release();
  }
};
// Get assignment marks data for report
export const getAssignmentMarks = async (req, res) => {
  try {
    ...
    if (assignmentNo) {
      query += ` AND Assignment_No = ?`;
      params.push(assignmentNo);
    }

    query += ` ORDER BY Register_Number, Assignment_No`;
    ...
    res.status(500).json({ error: 'Failed to fetch assignment marks' });
  }
};
...
export const updateAssignmentMark = async (req, res) => {
  try {
    ...
    if (!id || obtainedMark === undefined || obtainedMark === null) {
      return res.status(400).json({ error: 'ID and obtained mark are required' });
    }
    ...
    const [existing] = await pool.query(
      'SELECT Max_Marks FROM assignment_mark_entered WHERE id = ?',
      [id]
    );
    ...
    if (obtainedMark > maxMarks) {
      return res.status(400).json({ error: `Obtained mark cannot exceed ${maxMarks}` });
    }

    // Update the record
    await pool.query(
      'UPDATE assignment_mark_entered SET Obtained_Mark = ?, UpdatedAt = NOW() WHERE id = ?',
      [obtainedMark, id]
    );
    ...
    console.error('Error updating assignment mark:', error);
    res.status(500).json({ error: 'Failed to update assignment mark' });
  }
};
...
export const deleteAssignmentMark = async (req, res) => {
  try {
    ...
    const [existing] = await pool.query(
      'SELECT id FROM assignment_mark_entered WHERE id = ?',
      [id]
    );
    ...
    // Delete the record
    await pool.query('DELETE FROM assignment_mark_entered WHERE id = ?', [id]);
    ...
    console.error('Error deleting assignment mark:', error);
    res.status(500).json({ error: 'Failed to delete assignment mark' });
  }
};

// --- Report Filter Functions (from assignment_mark_entered table) ---
