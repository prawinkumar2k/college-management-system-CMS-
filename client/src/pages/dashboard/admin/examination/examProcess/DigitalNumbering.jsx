...
const generateDigitalNumberingHtml = (digitalData, examDate, session) => {
  ...
  digitalData.forEach((dept) => {
    dept.subjects.forEach((subject) => {
      const regNums = subject.register_numbers || [];
      const rows = [];
      for (let i = 0; i < regNums.length; i += 5) {
        rows.push(regNums.slice(i, i + 5));
      }

      const gridHtml = rows.map(row => `
        <div style="display: flex; justify-content: space-between; margin-bottom: 24px; font-size: 13px; font-family: monospace; font-weight: bold;">
          ${row.map(reg => `<div style="flex: 1; text-align: center;">${reg}</div>`).join('')}
          ${Array(5 - row.length).fill('<div style="flex: 1;"></div>').join('')}
        </div>
      `).join('');
      ...
      fullHtml += `
        <div class="page-container" style="page-break-after: always; width: 210mm; height: 296mm; padding: 10mm; box-sizing: border-box; background: white; margin: 0 auto; font-family: 'Times New Roman', Times, serif; overflow: hidden;">
          <div style="border: 2px solid #000; padding: 12px; height: 100%; display: flex; flex-direction: column; box-sizing: border-box;">
            ...
            <div style="text-align: center; margin-bottom: 20px; font-weight: 900; font-size: 15px; text-decoration: underline; text-transform: uppercase;">
...
const DigitalNumbering = () => {
  ...
  const handleView = () => {
    ...
    setLoading(true);
    api.get('/digitalNumbering/digital-numbering-report', {
      params: {
        examDate: date,
        session: session
      }
      ...
      .then(response => {
        const numberingData = response.data || [];
        const flatData = [];
        let serialNo = 1;

        numberingData.forEach(dept => {
          dept.subjects.forEach(subject => {
            subject.register_numbers.forEach(regNum => {
              flatData.push({
                sno: serialNo++,
                date: date,
                session: session,
  ...
  const handlePrint = () => {
    ...
    const html = generateDigitalNumberingHtml(digitalData, date, session);
        ...
        <div className="dashboard-main">
          ...
          <div className="dashboard-main-body p-24">
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-24">
              <h6 className="fw-semibold mb-0">Digital Numbering</h6>
            </div>
              ...
              <div className="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div>
                  ...
                  <span className="text-sm fw-medium text-secondary-light">
                    Select date and session to generate digital numbering
                  </span>
                </div>
                <button
                ...
                {showReport && (
                  <div className="mt-24 border-top pt-24">
                    <DataTable
                      ...
                      columns={columns}
                      loading={loading}
                      title={`Digital Numbering Preview - ${dateSession || ''}`}
                      enableExport={true}
                      enableActions={false}
                    />
                  </div>
