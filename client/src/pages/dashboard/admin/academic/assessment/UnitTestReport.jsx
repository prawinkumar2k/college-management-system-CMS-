...
const UnitTestReport = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('/api/unitTest/report/courses');
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      if (filters.courseName) {
        try {
          const response = await fetch(`/api/unitTest/report/departments?courseName=${encodeURIComponent(filters.courseName)}`);
          const data = await response.json();
          setDepartments(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSemesters = async () => {
      if (filters.courseName && filters.deptName) {
        try {
          const response = await fetch(`/api/unitTest/report/semesters?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}`);
          const data = await response.json();
          setSemesters(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchRegulations = async () => {
      if (filters.courseName && filters.deptName && filters.semester) {
        try {
          const response = await fetch(`/api/unitTest/report/regulations?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}`);
          const data = await response.json();
          setRegulations(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSections = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation) {
        try {
          const response = await fetch(`/api/unitTest/report/sections?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}`);
          const data = await response.json();
          setSections(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation && filters.section) {
        try {
          const response = await fetch(`/api/unitTest/report/subjects?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}`);
          const data = await response.json();
          setSubjects(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchUnitTests = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation && filters.section && filters.subjectName) {
        try {
          const response = await fetch(`/api/unitTest/report/unittests?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}&subjectName=${encodeURIComponent(filters.subjectName)}`);
          const data = await response.json();
          setUnitTests(data);
        } catch (error) {
  ...
  const handleFilterChange = (name, value) => {
    setFilters(prev => {
      ...
      if (name === 'subjectName') {
        ...
        // Reset unit test fields
        newFilters.testNo = '';
        newFilters.testDate = '';
        newFilters.maxMarks = '';
      }

      // Handle test number selection - auto-fill date and max marks
      if (name === 'testNo') {
        const selectedTest = unitTests.find(t => t.testNo === parseInt(value));
        if (selectedTest) {
          const date = selectedTest.assessmentDate ? new Date(selectedTest.assessmentDate).toLocaleDateString('en-GB') : '';
          newFilters.testDate = date;
          newFilters.maxMarks = selectedTest.maxMarks || '';
        }
      }
  ...
  const handleView = async () => {
    ...
    try {
      let url = `/api/unitTest/report/marks?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}`;
      ...
      if (filters.testNo) {
        url += `&testNo=${encodeURIComponent(filters.testNo)}`;
      }

      const response = await fetch(url);

      if (!response.ok) {
        throw new Error('Failed to fetch unit test marks');
      }

      const data = await response.json();
      setUnitTestMarks(data);
      setShowTable(true);

      if (data.length === 0) {
        toast.info('No unit test marks found for the selected filters');
      } else {
        toast.success(`${data.length} records loaded successfully`);
      }
    } catch (error) {
      console.error('Error fetching unit test marks:', error);
      toast.error('Failed to load unit test marks');
      setUnitTestMarks([]);
      setShowTable(false);
      setError('Failed to load unit test marks');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="dashboard-main">
          ...
          <div className="dashboard-main-body">
            ...
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
              <h6 className="fw-semibold mb-0">Unit Test Mark Report</h6>
            </div>

            {/* Standardized Print Content */}
            <div id="unittest-report" className="unittest-report-print-container" style={{ display: 'none' }}>
              <style>
                {`
                  @media print {
                    body * { visibility: hidden; }
                    #unittest-report, #unittest-report * { visibility: visible; }
                    #unittest-report {
                      position: absolute;
                      left: 0;
                      top: 0;
              ...
              {(() => {
                ...
                const chunks = chunkArray(unitTestMarks, ROWS_PER_PAGE);
                  ...
                  <div key={pageIdx} className="page-container" style={{ background: '#fff', fontFamily: "'Times New Roman', serif" }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', fontSize: '13px', color: "#000" }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div><strong>Test No:</strong> {filters.testNo || 'All'}</div>
                        <div><strong>Test Date:</strong> {filters.testDate || '-'}</div>
                      </div>
                    </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', color: "#000" }}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ border: '1px solid #222', padding: '8px', background: '#f4f4f4', textAlign: 'center' }}>Test No</th>
                      ...
                      <tbody>
                        {chunk.map((m, idx) => (
                          <tr key={m.id}>
                            ...
                            <td style={{ border: '1px solid #222', padding: '8px', textAlign: 'center' }}>{m.testNo}</td>
            ...
            <div className="card h-100 p-0 radius-12 mb-4">
              <div className="card-body p-24">
                <div className="row g-3">
                  ...
                  <div className="col-12 col-md-4">
                    <label className="form-label fw-semibold text-primary-light mb-8">
                      Unit Test Number (Optional)
                    </label>
                    <select
                      className="form-select radius-8"
                      value={filters.testNo}
                      onChange={(e) => handleFilterChange('testNo', e.target.value)}
                      disabled={!filters.subjectName}
                    >
                      <option value="">All Unit Tests</option>
                      {unitTests.map((test) => (
                        <option key={test.testNo} value={test.testNo}>
