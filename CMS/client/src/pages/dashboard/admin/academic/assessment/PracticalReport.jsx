...
const PracticalReport = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('/api/practicalMarks/report/courses');
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      if (filters.courseName) {
        try {
          const response = await fetch(`/api/practicalMarks/report/departments?courseName=${encodeURIComponent(filters.courseName)}`);
          const data = await response.json();
          setDepartments(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSemesters = async () => {
      if (filters.courseName && filters.deptName) {
        try {
          const response = await fetch(`/api/practicalMarks/report/semesters?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}`);
          const data = await response.json();
          setSemesters(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchRegulations = async () => {
      if (filters.courseName && filters.deptName && filters.semester) {
        try {
          const response = await fetch(`/api/practicalMarks/report/regulations?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}`);
          const data = await response.json();
          setRegulations(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSections = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation) {
        try {
          const response = await fetch(`/api/practicalMarks/report/sections?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}`);
          const data = await response.json();
          setSections(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation && filters.section) {
        try {
          const response = await fetch(`/api/practicalMarks/report/subjects?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}`);
          const data = await response.json();
          setSubjects(data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchPracticals = async () => {
      if (filters.courseName && filters.deptName && filters.semester && filters.regulation && filters.section && filters.subjectName) {
        try {
          const response = await fetch(`/api/practicalMarks/report/practicals?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}&subjectName=${encodeURIComponent(filters.subjectName)}`);
          const data = await response.json();
          setPracticals(data);
        } catch (error) {
  ...
  const handleFilterChange = (name, value) => {
    setFilters(prev => {
      ...
      if (name === 'practicalNo') {
        const selectedPractical = practicals.find(p => p.practicalNo === parseInt(value));
        if (selectedPractical) {
          const date = selectedPractical.assessmentDate ? new Date(selectedPractical.assessmentDate).toLocaleDateString('en-GB') : '';
          newFilters.practicalDate = date;
  ...
  const handleView = async () => {
    ...
    try {
      let url = `/api/practicalMarks/report/marks?courseName=${encodeURIComponent(filters.courseName)}&deptName=${encodeURIComponent(filters.deptName)}&semester=${encodeURIComponent(filters.semester)}&regulation=${encodeURIComponent(filters.regulation)}&section=${encodeURIComponent(filters.section)}`;

      if (filters.subjectName) {
        url += `&subjectName=${encodeURIComponent(filters.subjectName)}`;
      }

      if (filters.practicalNo) {
        url += `&practicalNo=${encodeURIComponent(filters.practicalNo)}`;
      }

      const response = await fetch(url);

      if (!response.ok) {
        throw new Error('Failed to fetch practical marks');
      }

      const data = await response.json();
      setPracticalMarks(data);
      setShowTable(true);

      if (data.length === 0) {
        toast.info('No practical marks found for the selected filters');
      } else {
        toast.success(`${data.length} records loaded successfully`);
      }
    } catch (error) {
      console.error('Error fetching practical marks:', error);
      toast.error('Failed to load practical marks');
      setPracticalMarks([]);
      setShowTable(false);
      setError('Failed to load practical marks');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    {
      accessorKey: 'practicalNo',
      header: 'Practical No',
      cell: ({ row }) => <div className="fw-medium">{row.original.practicalNo}</div>,
    },
    {
        ...
        <div className="dashboard-main">
          ...
          <div className="dashboard-main-body">
            ...
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
              <h6 className="fw-semibold mb-0">Practical Mark Report</h6>
            </div>

            {/* Standardized Print Content */}
            <div id="practical-report" className="practical-report-print-container" style={{ display: 'none' }}>
              <style>
                {`
                  @media print {
                    body * { visibility: hidden; }
                    #practical-report, #practical-report * { visibility: visible; }
                    #practical-report {
                      position: absolute;
                      left: 0;
                      top: 0;
              ...
              {(() => {
                ...
                const chunks = chunkArray(practicalMarks, ROWS_PER_PAGE);
                  ...
                  <div key={pageIdx} className="page-container" style={{ background: '#fff', fontFamily: "'Times New Roman', serif" }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', fontSize: '13px', color: "#000" }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div><strong>Practical No:</strong> {filters.practicalNo || 'All'}</div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', color: "#000" }}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ border: '1px solid #222', padding: '8px', background: '#f4f4f4', textAlign: 'center' }}>Prac No</th>
                      ...
                      <tbody>
                        {chunk.map((m, idx) => (
                          <tr key={m.id}>
                            ...
                            <td style={{ border: '1px solid #222', padding: '8px', textAlign: 'center' }}>{m.practicalNo}</td>
                ...
                <div className="row g-3">
                  ...
                  <div className="col-12 col-lg-3">
                    ...
                    <select
                      ...
                      onChange={(e) => handleFilterChange('practicalNo', e.target.value)}
                      disabled={!filters.subjectName}
                    >
                      <option value="">All Practicals</option>
                      {practicals.map((practical) => (
                        <option key={practical.practicalNo} value={practical.practicalNo}>
...
export default PracticalReport;
