...
export default function Ex2Present() {
  ...
  const handleView = async () => {
    ...
    try {
      ...
      const filtered = data.filter(item => {
        ...
        const isPresent = status === 'present' || status === 'p' || status.includes('present');
        return matchesDate && matchesSession && matchesSubject && isPresent;
      });
      ...
      const mapped = filtered.map((item, idx) => ({
        ...
        attendance: item.attendance_status || 'Present',
      ...
      setTableData(mapped);
      setShowTable(true);
      toast.success(`Loaded ${mapped.length} present records`);
    } catch (err) {
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'studName', header: 'Student Name' },
    {
      ...
      header: 'Attendance',
      cell: ({ getValue }) => <span className="badge bg-success">{getValue()}</span>
    }
  ], []);
        ...
        <div className="dashboard-main">
          ...
          <div className="dashboard-main-body" style={{ padding: 20 }}>
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
              <h6 className="fw-semibold mb-0">Ex2 Present Candidate List</h6>
            </div>
